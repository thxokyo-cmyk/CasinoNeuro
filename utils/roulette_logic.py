...
# ВАЖНО: включаем ТОЛЬКО визуально похожие цифры БЕЗ привязки к цвету
# Цвет проверяется отдельно в _validate_and_correct!
OCR_CONFUSION_MAP = {
    # OCR читает -> может быть на самом деле (визуально похожие)
    "0": ["0", "8", "6", "9"],
    "1": ["1", "7"],  # Убрал "4" - 1 и 4 НЕ похожи визуально!
    "2": ["2", "7"],
    "3": ["3", "8", "9"],
    "4": ["4", "9"],  # Убрал "1" - 4 и 1 НЕ похожи визуально!
    "5": ["5", "6", "8"],
    "6": ["6", "0", "8", "5"],
    "7": ["7", "1"],  # Убрал "2" - 7 и 2 не так уж похожи
    "8": ["8", "0", "6", "3"],
    "9": ["9", "0", "4"],  # Убрал "8" - 9 и 8 не так похожи
